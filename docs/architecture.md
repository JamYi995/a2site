...
| `@a2site/database` | 已实现 | PostgreSQL、PGlite、事务和版本化迁移 |
| `@a2site/identity` | 已实现 | 邮箱验证、Agent 凭证、作用域、轮换、撤销和审计 |
...
- 身份密钥只保存 HMAC 摘要，明文 Agent 凭证只在签发或轮换时返回一次。
- 验证码错误次数和来源限流写入数据库，不依赖单进程内存。
- 生产环境没有 PostgreSQL、正式哈希密钥或邮件适配器时启动失败关闭。
