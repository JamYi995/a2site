...
- 独立网关可通过 `A2SITE_ACTIONS_FILE` 加载网站业务动作；动作文件限制为站内端点，并拒绝符号链接、重复标识和未确认的高风险动作。
...
如需让独立网关公开网站已有业务能力，把动作数组写入服务端 JSON 文件并设置 `A2SITE_ACTIONS_FILE`。文件只描述动作，不得包含账号、令牌、验证码或连接串；正式环境必须使用绝对路径。
